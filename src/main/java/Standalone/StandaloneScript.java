...
public class StandaloneScript {
    ...
    String HIVE_HOME_REMOTE = new App().readhome("HIVE_HOME_REMOTE");
...
"export HIVE_HOME_REMOTE="+HIVE_HOME_REMOTE+"\n" +                 
...
"	do\n" +
"		echo $x\n" +
"		sudo cat $x >> $BASE_OUT/$TABLE/$TABLE.csv\n" +
"	done\n" +
"fi\n" +                
...
"$POSTGRES_HOME/psql -U "+username+" -d $DATABASE_OUT -c \"DROP TABLE IF EXISTS $TABLE_OUT;\"\n" +
"$POSTGRES_HOME/psql -U "+username+" -d $DATABASE_OUT -c \"CREATE TABLE IF NOT EXISTS $TABLE_OUT $SCHEMA;\"\n" +
"$POSTGRES_HOME/psql -U "+username+" -d $DATABASE_OUT -c \"\\COPY $TABLE_OUT FROM '$BASE_IN/$TABLE/$TABLE.csv' WITH DELIMITER AS '|';\"\n" +
...
"	chmod -R a+wrx $BASE_OUT\n" +
"fi\n" +                
"$POSTGRES_HOME/psql -U "+username+" -d $DATABASE -c \"\\COPY (SELECT * FROM $TABLE) TO '$BASE_OUT/$TABLE/$TABLE.csv' WITH (DELIMITER '|', FORMAT csv)\"\n" +
"#cp $BASE_OUT/tmp.csv $BASE_OUT/$TABLE\n" +
"#mv $BASE_OUT/$TABLE/tmp.csv $BASE_OUT/$TABLE/$TABLE.csv\n" +
"#mv $BASE_OUT/tmp.csv $BASE_OUT/$TABLE.csv\n"+ "\n";
        return Postgres2CSV;
    }
...
"$HADOOP_HOME/bin/hdfs dfs -rm -r $HDFS/$TABLE.csv\n" + "\n";
    ...
    public String Postgres_SQL(String postgres_SQL){
        String username = System.getProperty("user.name");
        String SQL = "$POSTGRES_HOME/psql -U "+username+" -d $DATABASE -c \""+postgres_SQL+"\"\n";
    ...
    public String TPCH_Postgres_SQL(Move_Data Data, String TPCH_SQL){
        ...
        String SQL = "$POSTGRES_HOME/psql -U "+username+" -d $DATABASE -f "+directory_operator+NameOp+"/"+NameOp+".sql"+"\n";
