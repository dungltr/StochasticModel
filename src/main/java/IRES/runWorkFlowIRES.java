...
public class runWorkFlowIRES {
    ...
    public static void deleteModel (Move_Data Data){
        String modelDirPath = OperatorFolder+Nameop(Data) + "/models";
        System.out.println("Directory will be deleted is: " + modelDirPath);
	File modelDir = new File(modelDirPath);
            ...
            //modelDir.delete();
                ...
                //Deleting the directory recursively using FileUtils.
                    ...
                    System.out.println("Directory has been deleted recursively !: " +modelDirPath);
...
*/        d.add("Optimization.random",Double.toString(TimeOfDay));
	...
	d1.writeToPropertiesFile(directory_datasets + d1.datasetName);
        ...
        d2.add("Constraints.Engine.SQL",Data.get_To());
	...
	d2.add("Optimization.size",Data.get_DataOutSize());  
        if (Data.get_To().toLowerCase().equals("postgres")){          
            d2.add("Optimization.page",Double.toString(size[size.length-2]));
            d2.add("Optimization.tuple",Double.toString(size[size.length-1]));
        }
	d2.writeToPropertiesFile(directory_datasets + d2.datasetName);  
...
*/        
    ...
    public void createOperatorMove(Move_Data Data, String SQL, double costEstimateValue, double[] parameter) throws IOException, Exception {
        String costExecution = Double.toString(parameter[0])
                                + " + (In0.size) * " + Double.toString(parameter[1])
                                + " + (In0.page) * " + Double.toString(parameter[2])
                                + " + (In0.tuple) * " + Double.toString(parameter[3])
                                + " + (In0.random) * " + Double.toString(parameter[4]);
        String node_pc = new App().getComputerName();
        String NameOp = Nameop(Data);
        String AbstractOp = "Abstract_"+NameOp;
        String AlgorithmsName = "move";
